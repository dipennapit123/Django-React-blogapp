...
} from "../features/authentications/authSlice";
    ...
    <div className="shadow py-5 sticky top-0 bg-white">
      <nav className="flex justify-between mx-20 h-8 items-center">
        <Link to="/" onClick={() => handleCategoryClick("all")}>
          <h1 className="font-extrabold text-3xl">MeroTechnologies Blogs</h1>
        </Link>
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4">
              <div className="text-sm flex gap-2">
                Hello{" "}
                <p className="underline text-midnight cursor-pointer">
                  @{user.username}
                </p>
              </div>
              <button
                onClick={handleLogout}
                className="bg-gradient-to-r from-pink-color to-pale-pink py-3 px-5 rounded-xl text-white font-semibold text-lg shadow-md shadow-pale-pink"
              >
                Logout
              </button>
            </div>
